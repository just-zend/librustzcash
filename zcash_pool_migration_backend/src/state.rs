...
impl MigrationState {
    ...
    pub fn next_broadcastable(&self, target_height: u32) -> Option<MigrationTxId> {
        self.transactions
            .iter()
            .find(|t| {
    ...
    pub fn mark_mined(&mut self, id: MigrationTxId, height: u32) {
        if let Some(tx) = self.transactions.iter_mut().find(|t| t.id == id) {
    ...
    pub fn next_step(&self, target_height: u32) -> AdvanceStep {
    ...
    /// schedule.
    pub fn transaction_statuses(&self, target_height: u32) -> Vec<TransactionStatus> {
        self.transactions
            .iter()
            .map(|t| {
...
mod tests {
    ...
    fn awaiting_signature_is_blocked_on_signature() {
        ...
        let views = state.transaction_statuses(100);
    ...
    fn next_broadcastable_respects_state_deps_and_schedule() {
        ...
        signed.scheduled_height = 5;
        let mut s = state_with(vec![tx(0, prep(0, 0), mined(10)), signed]);

        // Not due yet (target below scheduled height).
        assert_eq!(s.next_broadcastable(4), None);
        // Due and deps mined.
        assert_eq!(s.next_broadcastable(5), Some(MigrationTxId(1)));
        ...
        assert_eq!(s.next_broadcastable(5), None);
    }
    ...
    fn next_step_walks_the_lifecycle() {
        ...
        assert_eq!(s.next_step(100), AdvanceStep::Waiting);
        ...
        s.transactions[1].state = mined(11);
        assert_eq!(s.next_step(100), AdvanceStep::BuildTransfers);
    ...
    fn next_step_waiting_when_schedule_not_reached() {
        ...
        assert_eq!(s.next_step(20), AdvanceStep::Waiting);
        assert_eq!(
            s.next_step(50),
            AdvanceStep::Broadcast {
                id: MigrationTxId(1)
    ...
    fn mark_transitions_and_status() {
        ...
        s.mark_mined(MigrationTxId(0), 10);
        s.mark_mined(MigrationTxId(1), 11);
