...
//! adds the `pool_migrations` and `pool_migration_transactions` tables that persist that in-progress
//! migration, so a wallet resumes it after being closed or restarted.
//!
...
pub(super) const MIGRATION_ID: Uuid = zcash_pool_migration_sqlite::MIGRATION_ID;
...
impl RusqliteMigration for Migration {
    ...
    fn up(&self, transaction: &rusqlite::Transaction) -> Result<(), Self::Error> {
        ...
        zcash_pool_migration_sqlite::init_migration_tables(transaction)?;
        Ok(())
    }
